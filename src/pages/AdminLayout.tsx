...
function SidebarContent({ admin, onLogout, onNavClick, collapsed, onToggleCollapse }: { admin: AdminUser; onLogout: () => void; onNavClick?: () => void; collapsed: boolean; onToggleCollapse?: () => void }) {
  return (
    <div className="flex flex-col h-full relative w-full">
      <div className={`p-5 border-b border-coal flex items-center transition-all duration-300 ${collapsed ? 'justify-center' : 'gap-4 px-7'}`}>
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
        {navItems.map(item => (
          <NavLink
            ...
            onClick={onNavClick}
            className={({ isActive }) =>
              `flex items-center ${collapsed ? 'justify-center px-0' : 'gap-4 px-5'} py-3.5
      ...
      <div className={`p-5 border-t border-coal glass-panel pb-6 md:pb-5 ${collapsed ? 'flex flex-col items-center' : ''}`}>
        {!collapsed && (
          <div className="px-3.5 py-2.5 mb-4 bg-black/30 rounded-lg border border-coal/50 text-[11px] text-txt2 font-mono tracking-wide flex items-center gap-2.5 overflow-hidden">
        ...
        <button
          ...
          className={`w-full flex items-center justify-center gap-3 ${collapsed ? 'p-3 rounded-full' : 'px-5 py-3.5 rounded-xl'} border border-crimson/30 transition-all duration-300 text-sm font-medium text-crimson hover:bg-crimson/10 hover:shadow-[0_0_12px_rgba(255,42,85,0.2)]`}
        >
